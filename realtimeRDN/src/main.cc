...
void convertImage(cv::Mat& matSrcImage, cv::Mat& matProcessedImage, float fScale){
     float* v3fSrcImage = matSrcImage.ptr<float>(0);
	...
	for(ch = 0; ch < 3; ch++){
           pucProcessedImage[n][ch] = CutoffValue(v3fSrcImage[3*n+ch]*255.0f);
	}     
     }
...
int main(int argc, char* argv[])
{
   ...
   if (argc != 3) {
    ...
    //the RDN model input variable
    float* imageInputs = new float[inSize * batchSize];
    
    //the RDN model output variable
	...
	matInputImage = cv::Mat::zeros(inHeight, inWidth, CV_8UC3);
        //cout << "LINE "<< __LINE__ << endl;
        
	//define the parameters, which can convert the input image to RDN network
        int nRx = inWidth > matSrcImage.cols?(inWidth - matSrcImage.cols)/2: 0;
        int nRy = inHeight > matSrcImage.rows?(inHeight - matSrcImage.rows)/2: 0;
	int nRWidth = inWidth > matSrcImage.cols? matSrcImage.cols:inWidth;
	int nRHeight = inHeight > matSrcImage.rows? matSrcImage.rows:inHeight;
	...
	//Copy the source image to model input image
        matSrcImage.copyTo(matInputImage(cv::Rect(nRx, nRy, nRWidth, nRHeight)));
        
	//preprocess the model input image
        int r, c, ch;
        for(r = 0; r < inHeight; r++){
		cv::Vec3b* pv3bInputImage = matInputImage.ptr<cv::Vec3b>(r);
	   for(c = 0; c < inWidth; c++){
              for(ch = 0; ch < 3; ch++){
	        imageInputs[r*inWidth*3 + c*3 + ch] = pv3bInputImage[c][ch]/255.0; 
	...
	//load the model output to result image variables  
	...
	//Convert the Result variable to model output image
        matProcessedImage = cv::Mat(outHeight, outWidth*3, CV_32FC1, &(SRResult[0]));
        matResultYImage = cv::Mat(outHeight, outWidth, CV_8UC3);
	...
	//Set the parameters that convert result image from the model output size to disply size 
        nRx = outWidth > matSrcImage.cols*2?(outWidth - matSrcImage.cols*2)/2: 0;
        nRy = outHeight > matSrcImage.rows*2?(outHeight - matSrcImage.rows*2)/2: 0;
        nRWidth = outWidth > matSrcImage.cols*2? matSrcImage.cols*2:outWidth;
        nRHeight = outHeight > matSrcImage.rows*2? matSrcImage.rows*2:outHeight;

        //cout << "LINE "<< __LINE__ << endl;

        //Convert the result image size
        matResultImage = matResultYImage(cv::Rect(nRx, nRy, nRWidth, nRHeight));
        //cout << "LINE "<< __LINE__ << endl;
        //cout << "ResultImage width:" << matResultImage.cols << ", ResultImage height:" << matResultImage.rows <<endl;
        
	//Stop the postprocessing timing
        dPostProcessT = (static_cast<double>(cv::getTickCount())- dStartPostProcess)/ cv::getTickFrequency();
        //Output the post processing timing
