...
// based on  Vitis AI 1.4 VART SRNet demo code
...
void run_CNN(vart::Runner* runner) {
  ...
  for (unsigned int n = 0; n < hrimages.size(); n += batchSize) {
    ...
    for (unsigned int i = 0; i < runSize; i++) {
      ...
      resize(lrimage, image2, Size(inWidth, inHeight), 0, 0, INTER_CUBIC/*INTER_NEAREST*/);

      //convert the upscale images to Y channel iamges
      Mat matYCrCb;
      cvtColor(image2, matYCrCb, COLOR_BGR2YCrCb);
      cv::split(matYCrCb, vmatYCrCb);
      Mat matY = vmatYCrCb[0];
      cout << "Resize image width is:" << matY.cols << endl;
      cout << "Resize image width is:" << matY.rows << endl;

      /*Save the Y channel source images*/
      ...
      imwrite(cFileName, matY);
      
      //convert Y channel source image to input images
      for (int h = 0; h < inHeight; h++) {
	for (int w = 0; w < inWidth; w++) {
	    imageInputs[i * inSize+h*inWidth+w] = matY.at<uchar>(h, w)/255.0f;
        }
      }
    ...
    for (unsigned int i = 0; i < runSize; i++) {
      ...
      //extract the Y channel image of the comparison image 
      cv::cvtColor(hrimage, hrYCrCbimage, cv::COLOR_BGR2YCrCb);
      vector<Mat>vmatYCrCbimages;
      split(hrYCrCbimage, vmatYCrCbimages);
      
      //extract the result image from SRNet
      cv::Mat matProcessedImage = cv::Mat(outHeight, outWidth, CV_32FC1, &(FCResult[i*outSize]));
      cv::Mat matResultImage = cv::Mat(outHeight, outWidth, CV_8UC1);
      
      //convert to final result image
      convertImage(matProcessedImage, matResultImage, output_scale);

      //Figure the PSNR between the comparison Y channel image  and result Y channel image
      double dPSNR = FigurePSNR(matResultImage, vmatYCrCbimages[0]);
      ...
      //
      char cLogPicture[100];
      sprintf(cLogPicture, "ProcessedImage%d%d.txt", n, i);
      std::ofstream fout(cLogPicture, std::ios::out);
      int r, c;
      for(r=0; r < outHeight; r++){
         for(c = 0; c < outWidth; c++){
             fout << int(FCResult[i*outSize + r * outWidth + c]*255.0f) << " ";
